...
class UserService(IUserService):
    ...
    async def create_user(self, dto: UserRegisterDTO):
        ...
        if user is not None:
            raise ApiError.conflict('Пользователь с таким адресом электронной почты уже существует')
    ...
    async def request_code(self, email: str, host: str):
        ...
        await self.send_verification_code(UserVerificationData(
            ip=host,
            id=user.id,
            code=generate_code(),
            email=email,
    ...
    async def complete_register(self, dto: UserVerificationData):
        user = await self.__repository.find_by_email_and_code(email=dto.email, code=dto.code)
        if user is None:
        ...
        if code_info['ip'] != dto.ip:
            raise ApiError.forbidden(message='Код был отправлен не для этого устройства')
        if datetime.strptime(code_info['timestamp'], '%Y-%m-%d %H:%M:%S.%f') < datetime.now():
